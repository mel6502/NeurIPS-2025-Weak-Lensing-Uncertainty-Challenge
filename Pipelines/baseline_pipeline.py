...
class WLDataset(Dataset):
    """
    Torch dataset for weak-lensing images and labels, using a transform-based API
    identical to CosmologyDataset.

    Parameters
    ----------
    images : np.ndarray
        Array of shape (N, H, W), float32.
    labels : np.ndarray or None
        Array of shape (N, 2) for (Omega_m, S8), or None.
    transform : callable or None
        Transform applied to images (e.g. ToTensor + Normalize).
    label_transform : callable or None
        Transform applied to labels (e.g. StandardScaler or tensor conversion).
    """

    def __init__(self, images, labels=None, transform=None, label_transform=None):
        self.data = images.astype(np.float32)
        self.labels = labels
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            # match CosmologyDataset: return torch.Tensor (1,H,W)
...
def main():
    ...
    try:
        ...
        y_train_4d = all_labels[:, train_sys_idx, :]
        y_val_4d   = all_labels[:, val_sys_idx,   :]
