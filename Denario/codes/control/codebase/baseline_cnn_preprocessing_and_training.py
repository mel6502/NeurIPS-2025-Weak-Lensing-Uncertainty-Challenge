# filename: codebase/baseline_cnn_preprocessing_and_training.py
import os
import sys
import time
import json
...
class DataLoaderWL:
    ...
    def load(self):
        ...
        if labels.ndim != 3 or labels.shape[2] < 2:
            raise ValueError("Labels must have shape (Ncosmo, Nsys, >=5) or at least (Ncosmo, Nsys, 2) for cosmology.")
...
class SimpleCNN(nn.Module):
    ...
    Notes
    -----
    Input tensor shape: (batch, 1, H, W) with H >> W typical for survey stripes.
    """
...
def compute_score(true_vals, pred_means, pred_sigmas, lam=1e3, eps=1e-12):
    """
    Compute the challenge success score on CPU as defined (higher is better when negative sign is applied).
    ...
    -------
    float
        The averaged negative score across samples (the closer to 0 the better, typically <= 0).
    """
...
def main():
    ...
    2. Load data in starting-kit format, assemble masked maps into full images.
